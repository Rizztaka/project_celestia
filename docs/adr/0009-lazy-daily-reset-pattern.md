...
- **Negative:** If we ever need to send push notifications or emails *at the exact moment* of reset (e.g., "Your dailies have reset!"), this pattern will not support it, as it relies on the user initiating a request. We will defer background jobs until such a requirement arises.
