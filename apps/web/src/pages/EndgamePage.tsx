  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex gap-2">
        {[0, 1, 2, 3].map((i) => (
          <TeamSlot key={i} charKey={team[i]} onClick={() => onSlotClick(i)} saving={saving} />
        ))}
      </div>
    </div>
  );
...
function ChamberCard({
  ...
  return (
    <div className="bg-white/3 rounded-xl border border-white/10 p-4">
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <span
          className={`font-display min-w-[2.5rem] text-center text-3xl font-bold ${accentColor}`}
        >
          {value}
        </span>
        <button
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {(Object.keys(DIFFICULTY_CONFIG) as TheaterDifficulty[]).map((d) => {
            ...
            return (
              ...
              >
                {isActive && <div className={`absolute inset-0 ${cfg.bg} opacity-50 blur-xl`} />}
      ...
      <div className="glass-panel rounded-2xl p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-zinc-600">
            {cast.filter(Boolean).length} / 12 characters
          </span>
        </div>
                  ...
                  >
                    <div>
                      ...
                      <p className={`text-xs ${cfg.color}`}>
                        {cfg.label} · {r.actsCleared}/10 Acts
                      </p>
                    </div>
...
function AbyssView({ roster }: { roster: RosterCharacter[] }) {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {showAddCycleModal && (
        <AddCycleModal onAdd={handleAddCycle} onClose={() => setShowAddCycleModal(false)} />
      )}
    </div>
  );
}
  ...
  if (abyssError && abyssError instanceof ApiError && abyssError.status === 404) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-zinc-950 px-6 text-center">
        ...
        <Link
          to="/import"
          className="mt-2 rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-violet-500"
        >
          Go to Import
        </Link>
      </div>
  ...
  return (
    <div className="relative min-h-screen bg-zinc-950 text-zinc-300">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="bg-amber-900/8 absolute -bottom-64 -right-64 h-[600px] w-[600px] rounded-full blur-[120px]" />
      </div>
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        {/* Mode Tab Bar */}
        <div className="bg-white/3 mb-8 flex gap-2 rounded-2xl border border-white/10 p-1.5">
        ...
        {/* Active View */}
        {activeMode === 'ABYSS' ? <AbyssView roster={roster} /> : <TheaterView roster={roster} />}
      </main>
    </div>
  );
}
