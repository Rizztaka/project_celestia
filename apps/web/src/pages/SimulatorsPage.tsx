...
const RARITY_META: Record<PullResultItem['type'], {
  bg: string;
  border: string;
  text: string;
  glow: string;
...
function PullCard({ item, index }: { item: PullResultItem; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Avatar */}
      <div
        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border font-display text-sm font-bold ${meta.bg} ${meta.border} ${meta.text}`}
      >
        {initials}
      </div>

      {/* Name */}
      <p className={`w-full truncate text-[11px] font-semibold ${meta.text}`}>
        {name}
      </p>
...
function PullHistoryRow({ item, pullNumber }: { item: PullResultItem; pullNumber: number }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border px-4 py-2.5 ${meta.bg} ${meta.border} ${meta.glow}`}>
      <span className={`shrink-0 font-display text-xs font-bold ${meta.text}`}>#{pullNumber}</span>
      <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border font-display text-[10px] font-bold ${meta.bg} ${meta.border} ${meta.text}`}>
        {name.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <p className={`truncate text-sm font-semibold ${meta.text}`}>{name}</p>
        {item.isFeatured && (
          <p className="text-[10px] text-amber-500">Featured</p>
        )}
      </div>
...
function PityBar({ label, current, max, color }: { label: string; current: number; max: number; color: string }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-widest text-zinc-500">{label} Pity</span>
        <span className={`font-display text-sm font-bold ${isSoftPity ? 'text-amber-400' : 'text-white'}`}>
          {current} / {max}
          {isSoftPity && <span className="ml-1.5 text-[10px] font-semibold text-amber-500">Soft Pity!</span>}
        </span>
      </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {[
        ...
        { label: '5★ Pulled', value: `${stats.fiveStars} (${fiveStarRate}%)`, color: 'text-amber-400' },
        { label: '4★ Pulled', value: `${stats.fourStars} (${fourStarRate}%)`, color: 'text-violet-400' },
        { label: 'Featured 5★', value: stats.featuredFiveStars, color: 'text-amber-300' },
      ].map(stat => (
        <div key={stat.label} className="glass-panel flex flex-col gap-1 rounded-xl border border-white/5 p-4">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500">{stat.label}</p>
...
function BannerCard({ banner, selected, onSelect }: { banner: Banner; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button
      onClick={onSelect}
      disabled={isExpired}
      className={`glass-panel relative w-full cursor-pointer rounded-2xl border p-5 text-left transition-all duration-300 hover-lift ${
        selected
    ...
    >
      {/* Selected indicator */}
      {selected && (
        <span className="absolute right-4 top-4 flex h-5 w-5 items-center justify-center rounded-full bg-accent-500">
      ...
      <div className="mb-3 flex items-center gap-2.5 rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-amber-500/30 bg-amber-500/15 font-display text-xs font-bold text-amber-400">
          {featuredName.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase()}
        </div>
        <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {banner.fourStarKeys.map(key => (
          <span key={key} className="rounded-full border border-violet-500/20 bg-violet-500/10 px-2 py-0.5 text-[10px] font-semibold text-violet-400">
            {formatName(key)}
          </span>
        ))}
      </div>
      ...
      <div className="mt-3 flex items-center gap-1.5">
        ...
        <span className={`text-[11px] font-semibold ${isExpired ? 'text-red-500' : 'text-zinc-500'}`}>{daysLeft}</span>
      </div>
    </button>
  );
}
  ...
  return (
    <nav className="glass-panel sticky top-0 z-50 flex items-center justify-between border-b-0 border-white/10 px-6 py-4">
      ...
      <div className="flex items-center gap-6">
        <Link to="/intelligence" className="text-sm font-medium text-zinc-400 transition-colors hover:text-white">
          Intelligence
        </Link>
        <Link to="/endgame" className="text-sm font-medium text-zinc-400 transition-colors hover:text-white">
          Endgame
        </Link>
        <button
          onClick={onLogout}
...
export default function SimulatorsPage() {
  ...
  const [pullHistory, setPullHistory] = useState<(PullResultItem & { pullNumber: number; bannerId: string })[]>([]);
  ...
  const currentPity = selectedBannerId ? (pityMap[selectedBannerId] ?? { pity5: 0, pity4: 0, guaranteed5: false, guaranteed4: false }) : null;
  const currentStats = selectedBannerId ? (statsMap[selectedBannerId] ?? { total: 0, fiveStars: 0, fourStars: 0, threeStars: 0, featuredFiveStars: 0 }) : null;
  ...
  async function handlePull(count: 1 | 10) {
    ...
    try {
      ...
      // Update stats
      setStatsMap(prev => {
        const old = prev[selectedBannerId] ?? { total: 0, fiveStars: 0, fourStars: 0, threeStars: 0, featuredFiveStars: 0 };
        return {
          ...prev,
          [selectedBannerId]: {
            ...
            featuredFiveStars: old.featuredFiveStars + result.pulls.filter(p => p.type === '5_STAR' && p.isFeatured).length,
          },
        };
      });
  ...
  function handleReset() {
    ...
    setPityMap(prev => ({ ...prev, [selectedBannerId]: { pity5: 0, pity4: 0, guaranteed5: false, guaranteed4: false } }));
    setStatsMap(prev => ({ ...prev, [selectedBannerId]: { total: 0, fiveStars: 0, fourStars: 0, threeStars: 0, featuredFiveStars: 0 } }));
    setPullHistory(prev => prev.filter(p => p.bannerId !== selectedBannerId));
  }

  const notableHistory = pullHistory.filter(p => p.bannerId === selectedBannerId && p.type !== '3_STAR');
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-zinc-300">
          ...
          <>
            ...
            <section className="glass-panel mb-6 rounded-2xl border border-white/5 p-6">
              ...
              <div className="flex flex-col gap-4">
                <PityBar label="5★" current={currentPity.pity5} max={90} color="bg-gradient-to-r from-amber-500 to-yellow-400" />
                <PityBar label="4★" current={currentPity.pity4} max={10} color="bg-gradient-to-r from-violet-500 to-indigo-400" />
              </div>
            </section>
            ...
            <section className="mb-6 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <button
                ...
                className="group relative w-full overflow-hidden rounded-2xl bg-gradient-to-r from-accent-500 to-indigo-600 px-8 py-4 font-display text-base font-bold text-white shadow-lg shadow-accent-500/25 transition-all duration-300 hover:shadow-accent-500/40 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed sm:w-auto"
              >
              ...
              <button
                ...
                disabled={isSimulating}
                className="group relative w-full overflow-hidden rounded-2xl border border-amber-500/30 bg-amber-500/10 px-8 py-4 font-display text-base font-bold text-amber-300 shadow-lg shadow-amber-500/10 transition-all duration-300 hover:border-amber-400/50 hover:bg-amber-500/20 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed sm:w-auto"
              >
                <span className="relative z-10 flex items-center gap-2">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {latestBatch.length > 0 && (
              <section key={latestBatchKey} className="mb-6 animate-fade-in">
                <h2 className="mb-3 text-xs font-bold uppercase tracking-widest text-zinc-500">
                  Latest Pull
                </h2>
                <div className={`grid gap-2 ${latestBatch.length === 1 ? 'grid-cols-1 max-w-[120px] mx-auto' : 'grid-cols-5 sm:grid-cols-10'}`}>
              ...
              <section className="animate-fade-in">
                ...
                <div className="flex flex-col gap-2">
                  {notableHistory.slice(0, 20).map(item => (
                    <PullHistoryRow key={`${item.id}-${item.pullNumber}`} item={item} pullNumber={item.pullNumber} />
                  ))}
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        <div className="absolute -top-40 left-1/4 h-[500px] w-[500px] rounded-full bg-accent-500/5 blur-[120px]" />
